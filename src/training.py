...
def run_trainings(trainers, tokenizer):
    ...
    for trainer in trainers:
        ...
        trainer.train()
        trainer.save_model(f"./models/hf/{os.getenv('MODEL')}_{size}_{ep}_{lr}_{wd}")
        tokenizer.save_pretrained(f"./models/hf/{os.getenv('MODEL')}_{size}_{ep}_{lr}_{wd}")

        lora = get_peft_model(trainer.model, PEFT_CONFIG)
        lora.train()
        lora.save_pretrained(f"./models/lora/{os.getenv('MODEL')}_{size}_{ep}_{lr}_{wd}")
